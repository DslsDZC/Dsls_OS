...
// 添加虚拟内存映射桩函数
__attribute__((weak)) void* valloc(size_t size, int flags) {
    static uintptr_t next_addr = 0xFFFF800000000000; // 内核虚拟地址起始
    void* ret = (void*)next_addr;
    next_addr += size;
    return ret;
}

// 简化页表映射
__attribute__((weak)) void map_page(uintptr_t virt, uintptr_t phys, uint32_t flags) {
    // 实际应操作页表，此处仅桩实现
    *((volatile uint64_t*)virt) = phys | flags;
}
