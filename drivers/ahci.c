...
static int ahci_port_init(struct hba_port* port) {
    ...
    fis_base = alloc_phys_pages(1);
    
    // 设置命令列表基址
    uint64_t clb_addr = (uintptr_t)cl_base;
    port->clb = clb_addr & 0xFFFFFFFF;
    port->clbu = (clb_addr >> 32) & 0xFFFFFFFF;
    
    // 设置 FIS 基址
    uint64_t fis_addr = (uintptr_t)fis_base;
    port->fb = fis_addr & 0xFFFFFFFF;
    port->fbu = (fis_addr >> 32) & 0xFFFFFFFF;
